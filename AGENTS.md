...
- 2026-09-10 用户指示（六形环环部参考山手线双列规则・4.3.483）: 用户"环线部分是否可以参考山手线规则，然后这样可以给岔线流出空间"——六形环（大江户线）环部从"矩形周长排布"重构为**山手线式双列**（右列 14 + 左列 14，站名朝外、环内留白、垂直 pitch 按各列最高换乘 chip 自适应），光丘尾从环顶（Tochomae junction=右列顶）**独立垂直向上**，不再与环共享水平空间（旧 tail 在环左侧、站名被 clamp 到 40px 下限）。trains-page.js 六形环块重写：①尺寸——scale6 对齐山手线（移动 1.5/桌面 1.6）、loopRectW=110*scale6 窄矩形、loopRectH=pitch6*13（14 站 13 间隔，_colPitch6 同山手线 _colPitch）、尾区=10 站*spLoop6(24*scale6)+16、svgW=leftMargin+_nameW6+loopRectW+_nameW6+marginRight（_nameW6=GEOM.BRANCH_COL_W 移动/×1.2 桌面，移动端超容器时 clamp 到 _cw6Content）；②拓扑——右列(顶→底)=Tochomae+stations[37..25]（新宿..门前仲町）、左列(顶→底)=stations[11..24]（新宿西口..清澄白河），顶边相邻 Tochomae↔新宿西口、底边相邻 清澄白河↔门前仲町，环序闭合正确；③stationCoords 顺序仍=站表 index（尾 0..10 + 左列 11..24 + 右列 25..37，train 定位依赖 stationIndex 不受影响）；④_renderStationNode 删除旧六形环特判（left 朝右/right 朝左 + junctionX clamp）——新设计走通用 side 规则（左列朝左/右列与尾朝右），与山手线同一渲染路径；⑤routeElements 删除水平 stub，尾线直接从 junction 垂直向上。验证：verify_483_six.js 11 断言全过（站序/双列拓扑/底顶边相邻/junction 在右列顶/光丘在环顶上方/最小站名空间 96px≥90/尾环垂直分离）；integration_test.js 28 通过；node --check OK。※旧 tailAreaWidth/周长排布/水平 stub 全部移除；方向语义（光丘段 ▲/▼）与新几何兼容（站表 index 不变）。
