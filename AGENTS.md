...
- 2026-09-11 ユーザー指示（占用检测规则收窄到岔路・4.3.507）: 用户纠正 4.3.506 的适用范围——"我说的规定是针对那根岔"。占用检测 _pickSixLabelSide 只用于岔路站（光丘尾 10 站 + Tochomae junction，判定 o.x<junctionX || isJunction）；环站（左列 S32..S37/S11..S17、右列 S18..S31）恢复 4.3.504 固定规则（左列上方 y<junctionY 朝右、左列下方朝左、右列朝右）。视觉与 4.3.504/505 一致（岔路站占用检测结果与固定规则相同），但规则边界明确：岔路站名方向随占用动态变、环站方向固定。検証: node --check OK、岔路 11 站占用检测全 right、环站固定 right/left/right 枚举正确; LINE-DIAGRAM-SPEC 6.2/6.3/9 節同期。
