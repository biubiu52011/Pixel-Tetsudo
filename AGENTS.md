...
- 2026-09-10 用户拍板（线路图规定·环线标准・4.3.486）: 「山手线特例变成环线标准」——computeRouteGeometry 环线分支删除 isYamanote 特判，全部 type=loop 统一走双列展开布局（原山手线 30 站 15/15 双列、无底部中央站、中间开阔）。通用算法：站表按运行方向正序，从顶部切点 splitK 把环拉开成 U 形双列——右列 = splitK 逆序到 0 + N-1 逆序到 bottomA+1（内回方向），左列 = splitK+1 正序到 bottomA；顶部相邻对=(stations[splitK], stations[splitK+1])、底部相邻对=(stations[bottomA], stations[bottomA+1])。切点配置 RING_SPLIT_MAP（新设）：Yamanote=8（保持 JR 官方视觉：右列顶=田端/底=品川，左列顶=駒込/底=大崎）；未配置环线默认 splitK=floor(N/2)-1（左右均衡）。高度自适应（_colPitch 按每列最高换乘 chip 行数）与侧位渲染（_renderStationNode side=left/right）沿用。验证：山手线右列 [8..0,29..24]/左列 [9..23] 与旧布局完全一致（回归零）、N=27 奇数→右14左13、N=20 偶数→右10左10、node --check OK。※大江户线是六形环（isSixShapedLoop 分支）不受影响。
- 2026-09-10 用户指示（六形环双列·岔线不动・4.3.487）: 「岔线位置别动，只把环线部分替换成山手方案」——大江户线（isSixShapedLoop）环站由周长均匀排布改为山手式双列：左列 13 站（Tochomae 居中，上 6 下 6，站名朝右进环内）+ 右列 15 站（站名朝外到画布右缘）；垂直 pitch 按两列最高换乘 chip 自适应（_colPitch6，同山手线 _colPitch），环高 loopRectH=max(_pitch6*12, 200*scale6)；marginRight 加大（移动 +58/桌面 +64）给右列朝外站名让位。几何不动项：junction=环左缘中点、stubX、光丘尾 tailTotalHeight/tailStep（岔线位置零变化）；环序 0→左列上 1..6→右列 7..21→左列下 22..27→0 闭合。_renderStationNode 六形环三态 clamp：tail(o.x<junctionX)→朝右到 junctionX / 右列→朝外到 svgW-2 / 左列站→朝右到 loopCx（环内留白）；geometry 增 loopCx 字段。验证：verify_484_six.js 19/19（移动 svgW=394≤410、环宽 165、顶边站6↔站7同高、底边站21↔站22同高、Tochomae 左列中点、junction/stubX/tailTotalHeight 不变、右列朝外 78px≥70、左列 70.5px≥60、光丘尾 102px≥96；桌面环宽 176、右列朝外 86px、光丘尾 97.2px）、integration_test.js 28/28。※4.3.486 环线标准（RING_SPLIT_MAP 通用双列）明确排除六形环，两者互补不冲突。
- 2026-09-10 用户报告（环线列车“写着终点却越跑越远”・4.3.488）: 根因=4.3.486 环线双列重构后 stationCoords 数组顺序变为绘制顺序（右列逆序+左列正序），不再等于站表 index；updateTrainLayer 仍用 p.stationIndex（站表 index）当下标取坐标 → 全部 type=loop 非六形环线路（Yamanote 等）列车图标错位（Yamanote 30 站中 29 站错位：stationIndex=0 Tokyo 取到 Tabata）。六形环（Oedo）stationCoords 保持站表顺序故未受影响；直线线路亦不受影响。修复：updateTrainLayer 构建 stationId→coord 索引（svg.__coordIdx，跨增量更新复用），坐标定位改为站表 index→stationId→索引匹配，fallback 保留原下标逻辑（fusion 延伸线 baseIdx 偏移）。验证：verify_488.js Yamanote 新逻辑 0 错位（旧 29/30）、ChuoSobuLocal/Yokosuka 直线 0 错位、verify_484_six.js 19/19、integration_test.js 28/28、node --check OK。
