...
| Noda（東武アーバンパークライン）の Sakae（栄）駅 | P2 | 東武野田線に栄駅は実在しない（正しくは逆井 Sakasai、増尾 Masuo と高柳 Takayanagi の間）。外房線修正で Sakae の stationLines=[Noda] が残存し発見。駅名誤登録であり乗換に影響しないためユーザー判断待ち |
