...
| left（六形环岔路站：光丘尾 + Tochomae，v4.3.507 占用检测 _pickSixLabelSide） | 右：x+14/x+10；左：x-14/x-10 | y（central） | 右 start / 左 end |
| left（六形环环站：左列，v4.3.507 固定规则 y&lt;junctionY 上方朝右 / 下方朝左） | 上：x+10；下：x-14/x-10 | y（central） | 上 start / 下 end |
...
- **六形环（大江户线，v4.3.504 支线从环左/右侧 1/2 处展开）**：环段 27 站 + 都庁前分左右两列（14/14）——**都庁前（junction）在环左缘、左列第 7 位**（y=loopCy−0.5/14×rectH，中点偏上 36px，参考实际在环左半故从左侧展开）；左列（顶→下）= [麻布十番…新宿, 都庁前, 新宿西口…春日]，右列（顶→下）= [本郷三丁目…赤羽橋]。站序流 都庁前→左列下（春日）→环底→右列下（本郷三丁目）→右列上（赤羽橋）→环顶→左列上（麻布十番…新宿）→都庁前（闭合）。**光丘尾（10 站）从都庁前水平 stub 向左（stubX=leftMargin+10×scale6）后垂直向上**——10 站在环上半部左侧带（tail 顶 ≥ 边距）。**站名侧（v4.3.507）**：**占用检测只针对岔路站**（用户裁定"规定是针对那根岔"）——光丘尾 10 站 + Tochomae 用 `_pickSixLabelSide`（①空间放不下 16px 全尺寸文字宽 ②stub 线穿过 ③光丘尾站名带占用 ④对面圆点占用；左侧被占用→右、右侧被占用→左）；**环站（左列/右列）保持 4.3.504 固定规则**：左列上方朝右、左列下方朝左、右列朝右。当前视觉：光丘尾/左列上方/都庁前朝右、左列下方朝左、右列朝右。朝右的 left 站走窄空间 clamp（光丘尾到环左缘前 / 左列上方与都庁前到右列圆点前 58px 空间）。右列站名空间 = marginRight−16（移动 76 / 桌面 68px）。环高 = 山手线公式 + 换乘 chip 高度动态放大（_colPitch6）；svgH 公式 `max(rectH+120, 2×(marginTopBot+0.5/14×rectH+tailTotalHeight))`。
