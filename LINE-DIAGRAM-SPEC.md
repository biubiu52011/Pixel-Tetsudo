...
| left（六形环 tail 站，v4.3.502 仅光丘尾） | x+14/x+10 | y（central） | start |
| left（六形环左列，v4.3.502 双列） | x-14/x-10 | y（central） | end |
...
- **六形环（大江户线，v4.3.502 左右二分）**：环段 28 站分左右两列（14/14，与山手线同构）——左列顶=都庁前（junction），站序流 都庁前→左列下（森下）→环底→右列下（清澄白河）→右列上（新宿）→环顶闭合；光丘尾（10 站）保留，从都庁前（左列顶）水平 stub 后垂直向上。站名朝外（左列 anchor=end / 右列 anchor=start）；右列站名空间 = marginRight−16（移动 76 / 桌面 68px）。环高 = 山手线公式 + 换乘 chip 高度动态放大（_colPitch6）；svgH 须同时容纳环（垂直居中）与光丘尾（尾顶 ≥ 边距）。
- 站名 clamp 公式：双列左列 `tx-4`、右列 `svgW-2-tx`（通用）；光丘尾 `max(40, floor(junctionX-tx-4))`（946/951）；周长均布环内公式保留作防御。
