...
- **支线分叉布局（v4.3.515，用户指示"双支线或以上不要用弯折方案，用ㅕㅑ这类"）**：直线线型支线 **≥2 条**时左右交替分叉（ㅕㅑ 镜像）——偶数支线朝右（ㅑ）、奇数支线朝左（ㅕ）；单支线保持右侧弯折（现状）。右侧支线：bx=mainCx+BRANCH_STUB+右列序×BRANCH_COL_W、站名朝右（tx=bx+10，anchor=start，现状）；左侧支线：bx=mainCx−_branchStubL−左列序×BRANCH_COL_W（**_branchStubL=主干最大站名宽+22**，避开主干朝左站名带 gap 10）、站名朝左（tx=bx−10，anchor=end）、支线名朝左；**左侧连接线从 junction 圆点沿主干下移 20px 再水平分叉**（避开 junction 主干站名带 y±8）。svgW=mainCx+_rightNeed+20（_rightNeed=BRANCH_STUB+右列数余量+10+右支线最大站名宽+2），mainCx=max(_baseW/2, _leftNeed+20)；_baseW 主导时主线位置不变。适用：鶴見線（海芝浦右/大川左）、成田線（空港右/我孫子左）。
