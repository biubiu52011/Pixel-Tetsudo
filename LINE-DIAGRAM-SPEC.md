...
| left（六形环 tail 站，v4.3.503 环顶上方 o.y&lt;junctionY） | x+14/x+10 | y（central） | start |
| left（六形环左列环站与 Tochomae，v4.3.503 o.y≥junctionY） | x-14/x-10 | y（central） | end |
...
- **六形环（大江户线，v4.3.503 支线从环中心支出）**：环段 27 站分左右两列（左 14 / 右 13，与山手线同构）——**都庁前（junction）在环顶正中央**（x=loopCx 环水平中心线），站序流 都庁前→左列下（森下）→环底→右列下（清澄白河）→右列上（新宿=飯田橋段）→环顶→都庁前（闭合）；**光丘尾（10 站）从都庁前正上方垂直向上**（stubX=loopCx，无水平 stub 段）。站名：光丘尾朝右（clamp=svgW-2-tx，与右列站 y 带分离）、Tochomae 与左列站朝左（anchor=end）、右列站朝右（anchor=start）；右列站名空间 = marginRight−16（移动 76 / 桌面 68px）。环高 = 山手线公式 + 换乘 chip 高度动态放大（_colPitch6）；svgH 公式 `max(rectH+120, 2×(marginTopBot+rectH/2+tailTotalHeight))` 保证环垂直居中 + 尾顶 ≥ 边距。
- 站名 clamp 公式：双列左列 `tx-4`、右列 `svgW-2-tx`（通用）；光丘尾 `max(40, floor(svgW-2-tx))`（v4.3.503，946/953）；周长均布环内公式保留作防御。
