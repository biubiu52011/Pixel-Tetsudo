/* trains-geometry.js: 列车页几何计算（全局） */
  ...
  function computeRouteGeometry(line, lineId) {
    ...
    if (isSixShapedLoop && stations.length > 2) {
      ...
      var tailTotalHeight = tailCount > 0 ? Math.min(loopRectH, tailCount * _pitch6) + 32 : 0; // v4.3.884: 加回比例限制（≤loopRectH） // v4.3.883: 去掉 min(loopRectH*0.85) 限制（tail 实际高度超限时裁剪） // v4.3.878: spLoop6→_pitch6（支线竖列与环线统一站距）
