...
行高 = 18px/行（= ICON 16 + GAP 2 渲染口径，v4.3.851 由 22 收紧；见 5.4.1.1.3）；行数 = ⌈min(换乘数, 16) ÷ 4⌉（每行最多 4 个、行数上限 4 = RuntimeConfig.TRANSFER_MAX_ROWS，4×4=16 个图标上限，v4.3.849 用户裁定；溢出显示 "+n"）；一点空隙 = 28px（v4.3.852 由 34 收紧；其中 21 为固定几何 = chip 顶偏 14 + 圆点半径 7，不可再缩，可见留白 1 行 7 / 2 行 5 / 3 行 3 / 4 行 1px）。

站间距 = 全线各站换乘图标的高 的最大值 + 一点空隙（无换乘站按 1 行计，sp = 46 / 64 / 82 / 100，对应 1–4 行）。
...
• 站间距：换乘图标的高 + 一点空隙（行数×18 + 28：46/64/82/100；详见 5.4.1.2.2）。
