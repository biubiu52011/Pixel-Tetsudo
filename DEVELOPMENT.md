    ...
    - [5.4 线路图设计规定（SVG 渲染标准）](#54-线路图设计规定svg-渲染标准)
      - [5.4.1 基础前置规则（所有画法通用）](#541-基础前置规则所有画法通用)
        ...
        - [5.4.1.2 共有规则（所有画法必须遵守）](#5412-共有规则所有画法必须遵守)
          ...
          - [5.4.1.2.2 站间距与留白（分档）](#54122-站间距与留白分档)
...
• 站间距分档（按站数）：≤20 站 72/62px；21-30 站 66/58px；31-50 站 60/54px；>50 站 56/50px（移动/桌面）。
...
• 换乘 chip：ICON=16、GAP=2、每行上限 4（窄列 3/2）、行数上限 2、溢出 +n；chip 必须有一边与站名文字侧边对齐；顶部 ty+14（换乘）/ty+9（普通）。

• 字体：全局唯一 Fusion Pixel（回退 Courier New/monospace）；站名字号 16px（移动/桌面统一）。

• 站间距分档（站数→移动/桌面）：≤20→72/62；21-30→66/58；31-50→60/54；>50→56/50。
...
环状但非标准圆环形态——环段分左右两列（站序按 data/core 冻结数据均分，不依赖具体线路）、站序流闭合（起点 junction → 左列下 → 环底 → 右列下 → 右列上 → 环顶 → 左列上 → 闭合）、支线从环左/右侧 1/2 处展开。当前形态：六形环（此类线路本不多见；画法规则通用，不绑定任何线路）。布局：junction 位于环左缘（y=loopCy−0.5/列数×rectH，中点偏上；junction 位于环左半，故从左侧展开）；左列（顶→下）与右列（顶→下）站序由冻结数据按站序流规则分配。
