...
站间距 sp = **换乘图标的高 + 一点空隙**——图标的高 = 行数 × 行高（22px/行），保证换乘 chip 底部不与相邻站圆点/站名重叠。适用于**所有画法**（直线 / 环线双列 / 六形环 / 支线竖列；横排支线站距与右侧 stub 以 sp 为保底），移动/桌面统一，不分档。

行高 = 22px/行（换乘图标行，见 5.4.1.1.3）；行数 = ⌈min(换乘数, 16) ÷ 4⌉（每行最多 4 个、行数上限 4 = RuntimeConfig.TRANSFER_MAX_ROWS，4×4=16 个图标上限，v4.3.849 用户裁定；溢出显示 "+n"）；一点空隙 = 34px（= ICON 16 + 顶隙 6 + 底距 12，chip 底部与下一站圆点顶缘的可见空隙 17–23px）。

站间距 = 全线各站换乘图标的高 的最大值 + 一点空隙（无换乘站按 1 行计，保底 56px）。
...
• 站间距：换乘图标的高 + 一点空隙（行数×22 + 34，保底 56px；详见 5.4.1.2.2）。
