...
行高 = 18px/行（= ICON 16 + GAP 2 渲染口径，v4.3.851 由 22 收紧；见 5.4.1.1.3）；行数 = ⌈min(换乘数, 16) ÷ 4⌉（每行最多 4 个、行数上限 4 = RuntimeConfig.TRANSFER_MAX_ROWS，4×4=16 个图标上限，v4.3.849 用户裁定；溢出显示 "+n"）；一点空隙 = 34px（= ICON 16 + 顶隙 6 + 底距 12，chip 底部与下一站圆点顶缘的可见空隙 1 行 13 / 2 行 11 / 3 行 9 / 4 行 7px）。
...
• 站间距：换乘图标的高 + 一点空隙（行数×18 + 34：52/70/88/106；详见 5.4.1.2.2）。
