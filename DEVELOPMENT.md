...
行高 = 18px/行（= ICON 16 + GAP 2 渲染口径，v4.3.851 由 22 收紧；见 5.4.1.1.3）；行数 = ⌈min(换乘数, 16) ÷ 4⌉（每行最多 4 个、行数上限 4 = RuntimeConfig.TRANSFER_MAX_ROWS，4×4=16 个图标上限，v4.3.849 用户裁定；溢出显示 "+n"）；一点空隙 = 22px（v4.3.855 用户要求由 25 再收紧；G=22 时 2 行 chip 压下一站圆点 1px、3 行压 3px——用户试效果，接受轻微重叠）。

**全线统一**（v4.3.855，用户要求统一站间距，回退 v4.3.854 逐段）：站距 sp = **全线各站换乘图标的高** 的最大值 + 一点空隙——大站（多行 chip）抬高全线所有段，视觉间距均匀。无换乘站按 1 行计；sp = max(行数)×18 + 22（当前数据最高 3 行 → 全线 76）。
...
• 站间距：全线统一 = 全线最高换乘图标的高 + 一点空隙（max(行数)×18 + 22；详见 5.4.1.2.2）。
