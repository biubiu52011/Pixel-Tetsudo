...
> **文档结构**：第一部分 综合版（第 1-11 章）——项目概要、架构、规范、设计、API、部署、测试、风险、开发规则；第二部分 附录（附录 A-B）——源文档全文（权威细则）。
> **阅读建议**：新会话 / AI agent 建议顺序——1.1 文档目的与约定 → 3 系统架构 → 4 通用规范 → 第 11 章开发规则 → 5.4 线路图设计；涉及具体主题时按目录直达对应章节。
  ...
  - [5. 前端设计](#5-前端设计)
    ...
    - [5.4 线路图设计规定（SVG 渲染标准）](#54-线路图设计规定svg-渲染标准)
      - [5.4.1 基础前置规则（所有画法通用）](#541-基础前置规则所有画法通用)
        - [5.4.1.1 共有元素（所有画法必须具备）](#5411-共有元素所有画法必须具备)
          - [5.4.1.1.1 车站节点（圆点规格与站名偏移）](#54111-车站节点圆点规格与站名偏移)
          - [5.4.1.1.2 站名文本与锚点（通用规则）](#54112-站名文本与锚点通用规则)
          - [5.4.1.1.3 换乘图标 chip（规格与对齐）](#54113-换乘图标-chip规格与对齐)
          - [5.4.1.1.4 线路主线（线宽与透明度）](#54114-线路主线线宽与透明度)
          - [5.4.1.1.5 直通标记（箭头与留白）](#54115-直通标记箭头与留白)
          - [5.4.1.1.6 列车标签（方向端点站名）](#54116-列车标签方向端点站名)
        - [5.4.1.2 共有规则（所有画法必须遵守）](#5412-共有规则所有画法必须遵守)
          - [5.4.1.2.1 字体体系（字体族/字号刻度/字重颜色）](#54121-字体体系字体族字号刻度字重颜色)
          - [5.4.1.2.2 站间距与留白（换乘 chip 高度自适应）](#54122-站间距与留白换乘-chip-高度自适应)
          - [5.4.1.2.3 站名自适应（clamp）](#54123-站名自适应clamp)
          - [5.4.1.2.4 触控与间距（移动端）](#54124-触控与间距移动端)
          - [5.4.1.2.5 缩放系数（通用）](#54125-缩放系数通用)
          - [5.4.1.2.6 画布基准](#54126-画布基准)
          - [5.4.1.2.7 GEOM 设计令牌](#54127-geom-设计令牌)
          - [5.4.1.2.8 数据来源](#54128-数据来源)
      - [5.4.2 环线布局（画法专属）](#542-环线布局画法专属)
        - [5.4.2.1 标准环线（通用画法）](#5421-标准环线通用画法)
        - [5.4.2.2 异常环（通用画法）](#5422-异常环通用画法)
      - [5.4.3 支线布局（画法专属）](#543-支线布局画法专属)
        - [5.4.3.1 支线类型判定](#5431-支线类型判定)
        - [5.4.3.2 单支线（单短 ≤4 站 / 单长 >4 站）](#5432-单支线单短-4-站-单长-4-站)
        - [5.4.3.3 多支线全短（≥2 条，全部 ≤4 站）](#5433-多支线全短2-条全部-4-站)
        - [5.4.3.4 多支线全长（≥2 条，全部 >4 站）](#5434-多支线全长2-条全部-4-站)
        - [5.4.3.5 多支线长短不一（≥2 条，长短混合）](#5435-多支线长短不一2-条长短混合)
        - [5.4.3.6 支线几何参数](#5436-支线几何参数)
      - [5.4.4 线路与线宽（画法专属，主线通用）](#544-线路与线宽画法专属主线通用)
      - [5.4.5 图标体系（卡片侧）](#545-图标体系卡片侧)
      - [5.4.6 现状不统一项（待统一）](#546-现状不统一项待统一)
                                                      ...
                                                      - [三层数据架构规则（Three-Layer Data Architecture Rule）](#三层数据架构规则three-layer-data-architecture-rule)
                                                        - [无孤儿迁移规则（No Orphan-Generating Migration Rule）](#无孤儿迁移规则no-orphan-generating-migration-rule)
                                                          - [只读优先规则（Read-Only First Rule）](#只读优先规则read-only-first-rule)
                                                            - [模块核心规则（Module Main-Heart Rule）](#模块核心规则module-main-heart-rule)
                                                              - [发布门规则（Release Gate Rule）](#发布门规则release-gate-rule)
                                                                - [运行契约（Runtime Contract）](#运行契约runtime-contract)
  - [附录（源文档全文）](#附录源文档全文)
  - [附录 A 设计文档（Line-to-Line Service Relation Layer）](#附录-a-设计文档line-to-line-service-relation-layer)
  - [1. 问题陈述](#1-问题陈述)
  ...
  - [4. 全部线路关系映射](#4-全部线路关系映射)
    - [4.1 BRANCH_OF 关系（来自 branchOf 字段）](#41-branchof-关系来自-branchof-字段)
    - [4.2 已验证的 THROUGH_SERVICE 关系](#42-已验证的-throughservice-关系)
  ...
  - [8. 后续阶段计划](#8-后续阶段计划)
  - [9. 结论](#9-结论)
  - [附录 B 基本信息（README）](#附录-b-基本信息readme)
...
##### 5.4.1.1 共有元素（所有画法必须具备）

###### 5.4.1.1.1 车站节点（圆点规格与站名偏移）
...
###### 5.4.1.1.4 线路主线（线宽与透明度）

• 线路主线：线宽 5、透明度 1.0（各画法的衍生结构线见画法专属小节）。

###### 5.4.1.1.5 直通标记（箭头与留白）
...
• 站间距：sp = 换乘图标的高 + 一点空隙（共同规则，见 5.4.1.2.2），不再按站数分档。

• 顶部/底部留白：topP=18、botP=16（有直通标签时 +26）。

###### 5.4.1.2.2 站间距与留白（换乘 chip 高度自适应）

站间距 sp = **换乘图标的高 + 一点空隙**——图标的高 = 行数 × 行高（18px/行，= ICON 16 + GAP 2 渲染口径），保证换乘 chip 底部不与相邻站圆点/站名重叠。适用于**所有画法**（直线 / 环线双列 / 六形环 / 支线竖列；横排支线站距与右侧 stub 以 sp 为保底），移动/桌面统一，不分档。

行高 = 18px/行（= ICON 16 + GAP 2 渲染口径，v4.3.851 由 22 收紧；见 5.4.1.1.3）；行数 = ⌈min(换乘数, 16) ÷ 4⌉（每行最多 4 个、行数上限 4 = RuntimeConfig.TRANSFER_MAX_ROWS，4×4=16 个图标上限，v4.3.849 用户裁定；溢出显示 "+n"）；一点空隙 = 25px（v4.3.853 定不重叠极限：安全条件 G ≥ 21 + 2×(行数−1)——21 = chip 顶偏 14 + 圆点半径 7，2×(行数−1) = chip 行距；可见留白 1 行 4 / 2 行 2 / 3 行 0 贴边。G=21 会因行距未计入而 2 行压 2px、3 行压 4px）。

**逐段**（v4.3.854）：每段站距 = **上一站**换乘图标的高 + 一点空隙，不再全线统一取最大值——大站（多行 chip）只抬高它相邻两段，小站（1 行）段距更短。无换乘站按 1 行计；段距 = 43 / 61 / 79 / 97，对应上一站 1–4 行。

环线双列垂直站距（_colPitch / _colPitch6）与直线 sp 同一标准：列内间距 = 该列各站换乘图标的高最大值 + 空隙，两列取最大；超过基准环高时环高放大为 间距×(列站数−1)。

• 站名自适应：超宽时缩小字号（shrink, never clip），下限 10px；可用宽度由 data-clamp-avail 计算。

###### 5.4.1.2.3 站名自适应（clamp）
...
| 场景 | 移动 | 桌面 | 来源 |
| --- | --- | --- | --- |
| 线路图整体缩放（所有画法统一） | 1.5 | 1.6 | 532 |
| 站名/标记内 _sc6（所有画法） | 1.5 | 1.3 | 941-944 |

• 画布基准：移动端内容宽度 MOBILE_CONTENT_W=297；桌面端直线型 820；环线 rectW=48（移动 72 / 桌面 76.8）。
...
• 站间距：逐段 = 上一站换乘图标的高 + 一点空隙（行数×18 + 25：43/61/79/97；详见 5.4.1.2.2）。
...
| 两侧站名空间 | 恒定 75×scale |  |
| 列内垂直站距 | 换乘图标的高 + 一点空隙（同 5.4.1.2.2 共同规则） | _colPitch 复用 _chipPitch |
...
junction 的 left 站（站名居右）走窄空间 clamp（到右列圆点前）。右列站名空间 = marginRight−16。环高 = 标准环线基准环高 + 换乘 chip 高度动态放大（_colPitch6，列内站距同 5.4.1.2.2 共同规则）；svgH 计算 max(rectH+120, 2×(marginTopBot+环半高+tailTotalHeight))。
...
• 右列站名空间 = marginRight−16；环高 = 标准环线基准环高 + chip 高度动态放大（同 5.4.1.2.2 共同规则）。
