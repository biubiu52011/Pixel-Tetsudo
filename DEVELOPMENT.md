  ...
  - [5. 前端设计](#5-前端设计)
    ...
    - [5.4 线路图设计规定（SVG 渲染标准）](#54-线路图设计规定svg-渲染标准)
      - [5.4.1 基础前置规则（所有画法通用）](#541-基础前置规则所有画法通用)
          - [5.4.1.1 车站节点（圆点规格与站名偏移）](#5411-车站节点圆点规格与站名偏移)
          - [5.4.1.2 站名文本与锚点（通用规则）](#5412-站名文本与锚点通用规则)
          - [5.4.1.3 换乘图标 chip（规格与对齐）](#5413-换乘图标-chip规格与对齐)
          - [5.4.1.4 线路主线（线宽与透明度）](#5414-线路主线线宽与透明度)
          - [5.4.1.5 直通标记（箭头与留白）](#5415-直通标记箭头与留白)
          - [5.4.1.6 列车标签（方向端点站名）](#5416-列车标签方向端点站名)
          - [5.4.1.7 字体体系（字体族/字号刻度/字重颜色）](#5417-字体体系字体族字号刻度字重颜色)
          - [5.4.1.8 站间距与留白（换乘 chip 高度自适应）](#5418-站间距与留白换乘-chip-高度自适应)
          - [5.4.1.9 站名自适应（clamp）](#5419-站名自适应clamp)
          - [5.4.1.10 触控与间距（移动端）](#54110-触控与间距移动端)
          - [5.4.1.11 缩放系数（通用）](#54111-缩放系数通用)
          - [5.4.1.12 画布基准](#54112-画布基准)
          - [5.4.1.13 GEOM 设计令牌](#54113-geom-设计令牌)
          - [5.4.1.14 数据来源](#54114-数据来源)
...
#### 5.4.1.8 站间距（换乘 chip 高度自适应，所有画法共用）
