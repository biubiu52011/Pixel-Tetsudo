...
行高 = 18px/行（= ICON 16 + GAP 2 渲染口径，v4.3.851 由 22 收紧；见 5.4.1.1.3）；行数 = ⌈min(换乘数, 16) ÷ 4⌉（每行最多 4 个、行数上限 4 = RuntimeConfig.TRANSFER_MAX_ROWS，4×4=16 个图标上限，v4.3.849 用户裁定；溢出显示 "+n"）；一点空隙 = 28px（v4.3.856 用户裁定「4×4 不改、不折叠、稍微留点空」——3 行 chip 留 3px 缝 / 2 行 5 / 1 行 7，不压圆点；4×4 排布（每行 4、最多 4 行）维持不变）。

**全线统一**（v4.3.855，用户要求统一站间距，回退 v4.3.854 逐段）：站距 sp = **全线各站换乘图标的高** 的最大值 + 一点空隙——大站（多行 chip）抬高全线所有段，视觉间距均匀。无换乘站按 1 行计；sp = max(行数)×18 + 28（当前数据最高 3 行 → 全线 82）。
...
• 站间距：全线统一 = 全线最高换乘图标的高 + 一点空隙（max(行数)×18 + 28；详见 5.4.1.2.2）。
