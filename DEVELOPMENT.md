...
【硬规则 · 基础框架单一实现】框架层由全站唯一实现提供，任何页面不得自建副本、重复实现或私自改造：

- 框架层范围：`<head>`（四语 meta / 字体 / CSP / viewport）、header/nav、footer、语言切换（langSwitcher）、共享 CSS（style.css / lang-bar.css）、共享 JS 基础链（见 3.10.3 基础层）、加载/空/错误态。
- 页面层范围：`main` 主体区（tab-content 内）与页面专属 CSS/JS。
- 新增页面只复制骨架模板并填写主体区；不得为单个页面另立 header、导航、语言切换或基础脚本链实现。
...
下表仅列各页主体区差异；框架层（head/header/nav/footer/语言切换/基础脚本链）由 3.10.1 单一实现提供，各页不得重复实现。
...
共享框架资源（style.css / lang-bar.css / 基础 JS 链）升级：必须同步全部页面版本号保持一致，禁止各页自行维护或各自 bump
...
| 站名字号 | 16 |
| 支线站名 | 14 |
| 主线 stroke-width | 5（环线/直线）/ 4（支线延伸段） |
| chip ICON | 16 |
| chip GAP | 2 |
| 行高 ROW_H | 18 |
...
所有线路元素统一规格（各画法的衍生结构线见画法专属小节）：

| 元素 | 线宽 | 透明度 | 说明 |
| --- | --- | --- | --- |
| 所有线路元素 | 5 | 0.6 | 环线/直线/支线/直通 统一规格 |

#### 5.4.1.13 直通标记（箭头与留白）
直通标记样式（boxed label，v4.3.832 统一 Streamline Core Solid SVG 图标）：

| 元素 | 值 | 说明 |
| --- | --- | --- |
| 背景 | 线色 16% 透明 | 圆角矩形 rx=3 |
| 边框 | 线色，线宽 1 | 圆角矩形 |
| 箭头 | SVG path，线色，线宽 1.8（桌面）/ 2（移动） | 圆头圆角（stroke-linecap: round） |
| 箭头形状 | 线首→∧ / 线尾→∨ / 中间→→ | 不依赖字体字形 |
| 文字 | 直通线路名 | 线色 |

直通标记：标在直通关系的实际接续站（THROUGH_JOIN_STATIONS 门控：未定义=全部共用站兜底、空数组=不标、列站=仅这些站），站名带外侧 26px 留白（线首上/线尾下）。途经非接续站不标。

**同一接续站有多条直通线路（分叉）的堆叠规则**：一个站同时直通多条线时（如东京站同时直通高崎线+宇都宫线），多个 chip 必须沿直通方向依次错开，严禁重叠：

| 直通方向 | 堆叠方式 | 第 N 个 chip 偏移 |
| --- | --- | --- |
| up（线首上方） | 沿垂直方向向上逐行错开 | ly = 基准 y − N × (chip高 + 4) |
| down（线尾下方） | 沿垂直方向向下逐行错开 | ly = 基准 y + N × (chip高 + 4) |
| middle（线路中间侧方） | 沿垂直方向向下逐行错开 | ly = 基准 y + N × (chip高 + 4) |

N 从 0 开始（第 0 个在基准位）。同方向多 chip 共用同一个 chip 高 + 4px 行距。up/down 方向以站圆点圆心为水平基准居中（lx = 站圆点 x − chip宽/2）。v4.3.913 修复：up/down 方向此前漏了堆叠，多 chip 重叠成一团。
...
尾段（tail，站数由数据决定）：从 junction 水平 stub 向外侧垂直展开——站距 = 环站距 / 2（支线紧凑，不撑高全图）——stubX=max(leftMargin+10×scale, leftMargin+10+尾段最长站名宽)（竖线位置：尾段站名居左后，全尺寸文字左缘仍 ≥ leftMargin）。
