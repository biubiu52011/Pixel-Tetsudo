...
# Full validation of canonical candidate
checks = []
...
# 4. No lost source stations (except intentional dedup)
...
# 9. All lines have valid station lists
no_stations = [lid for lid, l in cand["lines"].items() if not l.get("stations")]
check("all_lines_have_stations", len(no_stations), 0, len(no_stations) == 0)

# 10. All durations match station count
dur_mismatch = sum(1 for lid, l in cand["lines"].items() if l.get("durations") and len(l["durations"]) != len(l.get("stations",[])))
check("durations_match_stations", dur_mismatch, 0, dur_mismatch == 0)
