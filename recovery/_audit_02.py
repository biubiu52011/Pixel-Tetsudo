...
for bf in backup_files:
    if os.path.exists(bf):
        h = hashlib.sha256(open(bf, \"rb\").read()).hexdigest().upper()
        ...
        results.append({
            \"file\": bf,
            \"sha256\": h,
            \"size_bytes\": os.path.getsize(bf),
            \"lines_count\": len(d.get(\"lines\", {})),
            \"stations_count\": len(d.get(\"stations\", {})),
            \"name_map_count\": len(d.get(\"name_map\", {})),
            \"stationLines_count\": len(sl) if sl else 0,
            \"lineStationOrder_count\": len(lso) if lso else 0,
            \"tourism_count\": len(d.get(\"tourism\", {})),
            \"has_stationLines\": \"stationLines\" in d,
            \"has_lineStationOrder\": \"lineStationOrder\" in d,
        })
...
out = {
    ...
    \"backup_analysis\": results,
    \"js_source_analysis\": {
        \"line-control-new.js@56b2cee\": {\"line_count\": len(lines_new), \"lines\": sorted(lines_new)},
        \"line-control.js@caeae43^\": {\"line_count\": len(lines_old), \"lines\": sorted(lines_old)},
    },
    \"git_history_for_data_files\": [x for x in git_log if x],
}
fpath = os.path.join(repo, \"recovery\", \"reports\", \"02_source_analysis.json\")
